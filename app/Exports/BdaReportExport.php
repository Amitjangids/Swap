...
class BdaReportExport implements FromCollection,WithHeadings,WithEvents
{
    protected $records;
      ...
      if (empty($columnName)) {
       ...
       foreach($this->records as $record){
        ...
        if($record->status == '1'){
          ...
          $result[] = array(
             'id'=>$i,
             'Beneficiary' =>$record->titleAccount,
             'IBAN' => $record->iban,
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class    => function(AfterSheet $event) use($styleArray,$styleArray1) {
                ...
                $cellRange = 'A'.$heighest_row.':J'.$heighest_row; 
                $event->sheet->setCellValue('E'. ($heighest_row), '=SUM(E2:E'.$last_row.')');
                $event->sheet->setCellValue('F'. ($heighest_row), '=SUM(F2:F'.$last_row.')');
                $event->sheet->setCellValue('G'. ($heighest_row), '=SUM(G2:G'.$last_row.')');
                $event->sheet->getDelegate()->getStyle($cellRange)->getFont()->setSize(10);
                $event->sheet->getStyle($cellRange)->ApplyFromArray($styleArray);

                $event->sheet->setCellValue('D'. ($heighest_row), 'Total');

                $event->sheet->getStyle('A'.$heighest_row.':J'.$heighest_row)->ApplyFromArray($styleArray1);
                $event->sheet->getStyle('A1:J'.$heighest_row)->ApplyFromArray($styleArray1);
