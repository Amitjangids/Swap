...
class IndividualReportExport implements FromCollection, WithHeadings, WithEvents
{
    protected $records;
        ...
        foreach ($this->records as $record) {
            ...
            if ($record->receiver_id == 0 && $record->receiver_mobile != '') {
                ...
                if (isset($record->recipientName) && !empty($record->recipientName)) {
                    $receiverName = ucfirst($record->recipientName . ' ' . $record->recipientSurname);
                    $receiverPhone = $record->recipientMsisdn;
                } elseif ($record->payment_mode == "TRANSAFEROUT" || $record->payment_mode == "CARDPAYMENT") {
                    $receiverName = isset($record->User->name) ? ucfirst($record->User->name) : 'N/A';
                ...
                if (isset($record->recipientName) && !empty($record->recipientName)) {
                    ...
                    $receiverName = ucfirst($admin->username) . 'asdf' ?? 'N/AW';
                    $receiverPhone = 'N/A';
                }
            } else {
                if (isset($record->recipientName) && !empty($record->recipientName)) {
                    ...
                    if ($record->payment_mode == "TRANSAFEROUT" || $record->payment_mode == "CARDPAYMENT") {
                        $receiverName = isset($record->User->name) ? ucfirst($record->User->name) : 'N/A';
                        $receiverPhone = ucfirst($record->User->phone);
                    } elseif (isset($record->Receiver->name)) {
            ...
            } else if ($record->payment_mode == "TRANSAFEROUT" || $record->payment_mode == "CARDPAYMENT") {
                $transactionFor = $record->description == "Denial - POS Purchase" ? "POS Denial" : $record->description;
            ...
            if (
                $record->payment_mode === "TRANSAFEROUT" ||
                $record->payment_mode === "CARDPAYMENT" || $record->description === "Card Issuance \ Activation Fee"
            ) {
                $amount = (
                    $record->description === "Denial - POS Purchase" ||
                    $record->description === "Manual PIN Change" || $record->description === "Card Issuance \ Activation Fee"
                )
            ...
            $result[] = array(
                ...
                "Balance" => $record->runningBalance ? $record->runningBalance : $record->afterBalance,
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) use ($styleArray, $styleArray1) {
                ...
                $cellRange = 'A' . $heighest_row . ':O' . $heighest_row;
                ...
                $event->sheet->getStyle('A' . $heighest_row . ':O' . $heighest_row)->ApplyFromArray($styleArray1);
                $event->sheet->getStyle('A1:O' . $heighest_row)->ApplyFromArray($styleArray1);
