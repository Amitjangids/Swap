...
class DashboardController extends Controller {
    ...
    return view('dashboard.dashboard', ['title' => $pageTitle,'wallet_balance'=>intval($wallet_balance),'opt_this_month'=>intval($opt_this_month),'successfull_transactions'=>intval($successfull_transactions),'failure_transactions'=>intval($failure_transactions),'total_transfer'=>CURR.' '.intval($total_transfer),'total_deposit'=>CURR.' '.intval($total_deposit),'total_fees'=>CURR.' '.intval($total_fees)]);
    }
    ...
    public function submitterDashboard(Request $request)
    {
        ...
        return view('dashboard.submitter_dashbaord', ['title' => $pageTitle,'wallet_balance'=> intval($wallet_balance),'opt_this_month'=>intval($opt_this_month),'successfull_transactions'=>intval($successfull_transactions),'failure_transactions'=>intval($failure_transactions),'total_deposit'=>CURR.' '.intval($total_deposit),'total_transfer'=>CURR.' '.intval($total_transfer), 'total_fees'=>CURR.' '.intval($total_fees)]);
    }
    ...
    public function approverDashboard()
    {
        ...
        $pageTitle = 'Dashboard';
        return view('dashboard.approver_dashbaord', ['title' => $pageTitle,'wallet_balance'=>intval($wallet_balance),'opt_this_month'=>intval($opt_this_month),'successfull_transactions'=>intval($successfull_transactions),'failure_transactions'=>intval($failure_transactions),'total_deposit'=>CURR.' '.intval($total_deposit),'total_transfer'=>CURR.' '.intval($total_transfer), 'total_fees'=>CURR.' '.intval($total_fees)]);
    }
    ...
    public function approverGetExcelList(Request $request)
    {
        ...
        if(Auth::user()->user_type=="Submitter")
        {
            ...
            ->where('trans_type', 1)
                ->sum('amount_value');
            // $totalCredit = str_replace('.', '', $totalCredit);
        ...
        $response = array(
            ...
            "iTotalDisplayRecords" => $totalRecordswithFilter,
            "aaData" => $data_arr,
            "totalCredit"=>floor($totalCredit),
           "totalDebit"=>floor($totalDebit),
        );
        echo json_encode($response);
        die;
    }
    ...
    public function pendingGetExcelList(Request $request)
    {
        ...
        foreach ($records as $record) {
            ...
            $deleteText = ($userType == "Submitter") ? "Delete" : "Reject";
            $action = '
                <a href="'.$record->id.'" data-bs-toggle="modal" data-bs-target="#transList" class=""><i class="fa fa-eye" aria-hidden="true" title="View"></i></a>' .
                ($record->type == 0 ? '<a href="'.PUBLIC_PATH.'/assets/front/excel/'.$record->excel.'" class="" download="'.$record->excel.'"><i class="fa fa-download" aria-hidden="true" title="Download Excel"></i></a>' : '') .
                '<a id="approveExcelLink"  href="'.HTTP_PATH.'/approve-excel/'.$encId.'" class="approve-btn approveExcelLink">
                    <i class="fa fa-check" aria-hidden="true" title="Approve Excel"></i>
                </a>' .
            ...
            $data_arr[] = array(
                ...
                "excel"=>$record->excel,
                "no_of_records" => $record->no_of_records,
                "updated_at" => date('d M,y',strtotime($record->updated_at)),
                "total_fees" => CURR.' '.$record->total_fees,
                "totat_amount" => CURR.' '.$record->totat_amount,
                "status" =>$this->getStatus($record->status),
                "action" =>$action,
        ...
        { 
            ...
            //   echo '<pre>';print_r($excel_transaction);exit;
              ...
              foreach($excel_transaction as $key=>$row)
              {
                ...
                {
                    ExcelTransaction::where('id',$row->id)->update(['remarks'=>'Your monthly transfer limit has been reached.']);
                    $n_inserted_record +=1;
                    continue;
                } 
                ...
                {
                    ExcelTransaction::where('id',$row->id)->update(['remarks'=>'Your weekly transfer limit has been reached.']);
                    $n_inserted_record +=1;
                    continue;
                }
                ...
                {
                    ExcelTransaction::where('id',$row->id)->update(['remarks'=>'Your daily transfer limit has been reached.']);
                    $n_inserted_record +=1;
                    continue;
                }
                ...
                // { 
                    ...
                    { 
                            ...
                            {
                                ExcelTransaction::where('id',$row->id)->update(['remarks'=>'The minimum transfer amount should be greater than '.CURR.' '.$unverifiedKycMin]);
                                $n_inserted_record +=1;
                                continue;
                            }
                            ...
                            {
                                ExcelTransaction::where('id',$row->id)->update(['remarks'=>'You cannot transfer more than '.CURR.' '.$unverifiedKycMax.' because your KYC is still pending.']);
                                $n_inserted_record +=1;
                                continue;
                            }
                    }
                            ...
                            if($unverifiedKycMax < $amount)
                            {
                                ...
                                ExcelTransaction::where('id',$row->id)->update(['remarks'=>'You cannot transfer more than '.CURR.' '.$unverifiedKycMax.' because your KYC is not verified.Please verify your KYC first']);
                                $n_inserted_record +=1;
                                continue;
                            }
                            // echo"hello";die;
                ...
                // {
                    ...
                    if(isset($is_receiver_exist))
                    {
                        ...
                        if ($user_id == $is_receiver_exist->id) {
                            $msg='You cannot send funds to yourself';
                        ...
                        else
                        {
                            ...
                            {
                                ExcelTransaction::where('id',$row->id)->update(['remarks'=>'You cannot transfer less than '.CURR.' '.$transactionLimit->minSendMoney]);
                                $n_inserted_record +=1;
                                continue;
                            }
                            ...
                            {
                                ExcelTransaction::where('id',$row->id)->update(['remarks'=>'You cannot transfer more than '.CURR.' '.$transactionLimit->maxSendMoney]);
                                $n_inserted_record +=1;
                                continue;
                            }

                            $total_fees = 0;
                            $trans_fees = Transactionfee::where('transaction_type','Send Money')->where('min_amount', '<=', $amount)->where('max_amount', '>=',  $amount)->first();
                            if(!empty($trans_fees))
                            {
                            ...
                            if ($amount > $senderUser->wallet_balance) {
                                ...
                                // ->where('receiver_id', $is_receiver_exist->id)
                                // ->where('amount', $amount)
                                // // Add additional conditions as needed to uniquely identify a transaction
                                // ->first();
                                ...
                                $sender_wallet_amount = $senderUser->wallet_balance - $total_amount_to_transfer;
                       ...
                       try {
                            ...
                            {
                                ExcelTransaction::where('id',$row->id)->update(['remarks'=>'You cannot transfer less than '.CURR.' '.$transactionLimit->gimacMin]);
                                $n_inserted_record +=1;
                                continue;
                            }
                            ...
                            {
                                ExcelTransaction::where('id',$row->id)->update(['remarks'=>'You cannot transfer more than '.CURR.' '.$transactionLimit->gimacMax]);
                                $n_inserted_record +=1;
                                continue;
                            }
                            ...
                            if ($e instanceof \GuzzleHttp\Exception\RequestException && $e->hasResponse()) {
                                ...
                                if ($jsonResponse && isset($jsonResponse['error_description'])) {
                                    ...
                                    Issuertrxref::create(['issuertrxref'=>$next_issuertrxref,'messages'=>'Unable to extract error_description']);
                                    ExcelTransaction::where('id',$row->id)->update(['remarks'=>'Unable to extract error_description']);
                                    $n_inserted_record +=1;
                                }
                            } else {
                                ExcelTransaction::where('id',$row->id)->update(['remarks'=>'gimac server error']);
                                $n_inserted_record +=1;
                            }
                        }
                       }
            ...
            if($n_inserted_record > 0)
            {
                ...
                Session::put('error_message','We got error in '.$n_inserted_record.' records while transfer.');
            }

            UploadedExcel::where('id',$id)->update(['status'=>5]);
            ...
            if($t_inserted_record > 0)
            {
                ...
                Session::put('success_message',$t_inserted_record.' records inserted successfully!');
            }
...
// exit;
        ...
        elseif($user_type=="Approver")
        {
            ...
            Session::put('success_message','Record have been successfully approved and are visible in the merchant account.');
            return back();
        }
        else{
            UploadedExcel::where('id',$id)->update(['status'=>1,'approved_by'=>Auth::user()->id]);
            ExcelTransaction::where('excel_id',$id)->update(['approved_by'=>Auth::user()->id]);
            Session::put('success_message','Record have been successfully approved and are visible in the approver account.');
            return back();
        }  
    } 
        ...
        if(!empty($input))
        {
            ...
            if ($_POST['option'] !== 'swap_to_swap') {
                ...
                $customMessages = [
                    ...
                    'wallet_manager_id.required' => 'Wallet manager field can\'t be left blank',
                ];
            ...
            { 
                ...
                { 
                        ...
                        {
                            Session::put('error_message','The minimum transfer amount should be greater than '.CURR.' '.$unverifiedKycMin);
                            return Redirect::to('/single-transfer');
                        }
    
                        if($unverifiedKycMax < $request->amount)
                        {
                        Session::put('error_message','You cannot transfer more than '.CURR.' '.$unverifiedKycMax.' because merchant KYC is still pending.');
                        return Redirect::to('/single-transfer');
                        }
                }
                else{
                        ...
                        if($unverifiedKycMax < $request->amount)
                        {
                            Session::put('error_message','You cannot transfer more than '.CURR.' '.$unverifiedKycMax.' because merchant KYC is not verified.Please verify your KYC first');
                            return Redirect::to('/single-transfer');
                        }
                    }
            }
            ...
            if ($_POST['option'] == 'swap_to_swap') {
                ...
                {
                    Session::put('error_message','You cannot transfer less than '.CURR.' '.$transactionLimit->minSendMoney);
                    return Redirect::to('/single-transfer');
                }
        
                if($request->amount > $transactionLimit->maxSendMoney)
                {
                    Session::put('error_message','You cannot transfer more than '.CURR.' '.$transactionLimit->maxSendMoney);
                    return Redirect::to('/single-transfer');
                }
                ...
                {
                    Session::put('error_message',"The receiver's mobile is not registered with Swap.");
                    return Redirect::to('/single-transfer');
                }
                if(isset($is_receiver_exist))
                {
                ...
                {
                    Session::put('error_message','You cannot transfer less than '.CURR.' '.$transactionLimit->gimacMin);
                    return Redirect::to('/single-transfer');
                }
        
                if($request->amount > $transactionLimit->gimacMax)
                {
                    Session::put('error_message','You cannot transfer more than '.CURR.' '.$transactionLimit->gimacMax);
                    return Redirect::to('/single-transfer');
                }
            ...
            ExcelTransaction::create([
             ...
             Session::put('success_message','Single transaction has been saved successfully');
             return Redirect::to('/pending-approvals');

        }

       
        return view('dashboard.single_transfer', ['title' => $pageTitle,'country_list'=>$country_list,'wallet_manager_list'=>$wallet_manager_list]);
    }
    ...
    public function bulkTransfer(Request $request)
    {
        ...
        if(!empty($input))
        {
            ...
            $customMessages = [
                ...
                'excel_file.mimes' => 'Excel file should be in xlsx,xls',
            ];
            ...
            // $image='90897125_SwapWalletNew.xlsx';
            ...
            if ($errorCount > 0) {
                $errorMessage = "The uploaded file is missing required data. Please ensure all necessary fields are filled out.";
            ...
            {
                unlink($path);
                Session::put('error_message','You cannot transfer less than '.CURR.' '.$transactionLimit->bulkMin);
                return Redirect::to('/bulk-transfer');
            }
            ...
            {
                unlink($path);
                Session::put('error_message','You cannot transfer more than '.CURR.' '.$transactionLimit->bulkMax);
                return Redirect::to('/bulk-transfer');
            }
            ...
            if($rowCount <=0)
            {
                Session::put('error_message','You cannot upload empty file');
                return Redirect::to('/bulk-transfer');
            }
            ...
            if (array_diff($header_array, $header_row) === array()) {
                ...
                foreach($excelData as $data)
                {
                    ...
                    if(isset($is_receiver_exist))
                    {
                        $trans_fees = Transactionfee::where('transaction_type','Money Transfer Via GIMAC')->where('min_amount', '<=', $data['amount'])
                        ...
                        if($trans_fees)
                        {
                            ...
                            {
                                $total_fees = number_format(($totalAmount * $trans_fees->fee_amount / 100), 2, '.', '');
                            }
                            $fees_arr[]=$total_fees;
                        }
                    }
                ...
                Session::put('success_message','Excel has been uploaded successfully');
                return Redirect::to('/pending-approvals');
            }
            else{
                Session::put('error_message','You cannot change the header row');
            }
