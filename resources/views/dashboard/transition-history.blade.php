    ...
    <div class="container">
        <div class="row mb-5">
           ...
           <div class="col-lg-6">
               <div class="small-box bg-green">
                    <div class="inner">
                        <h3 id='someElement1'>{{ CURR }} 0</h3>
                        <p>{{__('message.Total Debit')}}</p>
                    </div>
                </div>
           </div>
        </div>
            <div class="row mb-4">
               <div class="col-lg-4 col-12 col-md-8 col-sm-8 col-xs-8">
            	    <div class="search_box custom-daterangepicker">
            			<input type="text" class="form-control" id="CustomSearchTextField" placeholder="{{__('message.Search by Txn. Reference number')}}">
...
var englishLocale = {
 ...
 var oTable = $('#exampleTable').DataTable({
            ...
            order: [[4, 'desc']],
            ajax:{
                url: "{{HTTP_PATH}}/approver-get-excel-list",
                type: "GET", // or "GET" depending on your server-side code
                dataType: "json",
                dataSrc: function(response) {
                    var totalCredit = response.totalCredit;
                    var totalDebit = response.totalDebit;
                    // totalCredit = totalCredit.replace(/,/g, '');
                    // totalDebit = totalDebit.replace(/,/g, '');

                        $('#someElement').html('{{ CURR }} ' + totalCredit);
                        $('#someElement1').html('{{ CURR }} ' + totalDebit);
        ...
        var url = '{{ route("export") }}?search=' + searchValue + '&daterange=' + dateRange;
    ...
    $('.export_excel').on('click',function(){
                // alert('ok');
                window.location.href='{{HTTP_PATH}}/export_excel';
            });
