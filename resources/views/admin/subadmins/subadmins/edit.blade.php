                                ...
                                <div class="sizes-drop" style="display: none1;" id="sizes-dropdown">
                                    ...
                                    <div class="sizemorescroll">
                                        <div class="cloth_size"> 
                                            <?php 
                                            $roles_values = explode(',',$recordInfo->role_ids);
                                            global $roles;
                                            foreach ($roles as $roleKey => $roleVal) {
                                                $selected = '';
                                                if(in_array($roleKey, $roles_values)){
                                                    $selected = 'checked="checked"';
                                                }
                                                ?>
                                                <div class="des_box_cont test-size mmbtm">
                                                    <div class="main_role"><input type="checkbox" {{$selected}} id="StrategyAsset{{$roleKey}}" value="{{$roleKey}}" name="role_ids[]" onclick="checkSubRole({{$roleKey}})"><label>{{$roleVal}}</label></div>
                                                </div>
                                            <?php }
                                            ?>
