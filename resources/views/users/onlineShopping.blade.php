            ...
            <div class="modal-dialog">
                ...
                <div class="modal-content">
                    <div class="pin-box">
                        ...
                        <h3 id="pay_error_message">{{__('message.Success!')}}</h3>
    ...
    <div class="row">
        <div class="col-sm-6 ml-auto mr-auto">
            <div class="order-summery">
                <table>
                    <tr><th colspan="2">{{__('message.Order Summery')}}</th></tr>
                    <tr><td>{{__('message.Merchant Name')}}</td><td>{{$orderInfo->Merchant->business_name}}</td></tr>
                    <tr><td>{{__('message.Order id')}}</td><td>{{$orderInfo->order_id}}</td></tr>
                    <tr><td>{{__('message.Bill Amount')}}</td><td>IQD {{$orderInfo->amount}}</td></tr>
                </table>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-sm-6">
            <div class="qr-box-payment">
                <h4>{{__('message.Scan QR code')}} <br>
                    to pay</h4>
                <div class="qr-inner">
                    {{HTML::image('public/'.$orderInfo->qr_code, SITE_TITLE)}}
                </div>
                <ul>
                    <li>1). {{__('message.Open your SatPay App')}}</li>
                    <li>2). {{__('message.Tap on online shopping')}}</li>
                    <li>3). {{__('message.Scan QR code')}}</li>
                </ul>
            </div>
        </div>
        ...
        <div class="col-sm-6">
            <div class="log-payment-box">
                <h5>{{__('message.Login to pay')}}</h5>
                <p>{{__('message.Enter your SatPay mobile number and password to make the payment')}} </p>
                ...
                <button type="submit" class="btn-grad grad-two">{{__('message.Pay Now')}}</button>
                {{ Form::close()}}
            </div>
        </div>
    </div>
