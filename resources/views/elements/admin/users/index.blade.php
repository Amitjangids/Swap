                        ...
                        @foreach($allrecords as $allrecord)
                            <tr>
                                ...
                                <td data-title="KYC Status">{{ ucfirst($allrecord->kyc_status) }}
                                    <!-- @if($allrecord->kyc_status == 1)
                                                    Approved
                                                    @elseif($allrecord->kyc_status == 2)
                                                    Declined
                                                    @elseif($allrecord->kyc_status == 3)
                                                    Not Submitted
                                                    @else
                                                    Pending
                                                    @endif -->
                                    ...
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu pull-right">
                                            ...
                                            @if(in_array('edit-users', $permissions))
                                                <li><a href="{{ URL::to('admin/users/edit-users/' . $allrecord->slug)}}"
                                            ...
                                            @if(in_array('payclient', $permissions))
                                                ...
                                                <li><a href="{{ URL::to('admin/users/payclient/' . $allrecord->slug)}}"
                                                        title="Pay User" class=""><i class="fa fa-money"></i>Pay User</a></li>
                                            @endif
                                            @if($allrecord->cardActiveInactive != "")
                                            @if(in_array('payclient', $permissions))
                                                <li><a href="{{ URL::to('admin/users/payclientrebate/' . $allrecord->slug)}}"
                                                        title="Pay Rebate" class=""><i class="fa fa-money"></i>Pay Rebate</a></li>
                                            @endif
                                            @endif
                                            <li><a href="#info{!! $allrecord->id !!}" title="View User Detail" class=""
                                                    rel='facebox'><i class="fa fa-eye"></i>View User Detail</a></li>
                                            @if($allrecord->gabonStampImg != "")
                                                <li><a href="{{ URL::to('admin/users/gabon-visa-stamp/' . $allrecord->slug)}}"
                                                    title="Gabon Visa Stamp" class=""><i class="fa fa-file"></i>Gabon Visa Stamp</a>
                                                </li>
                                            @endif

                                            @if(in_array('kycdetail', $permissions))
                                                <li><a href="{{ URL::to('admin/users/kycdetail/' . $allrecord->slug)}}"
                                                        title="View KYC Details" class=""><i class="fa fa-file"></i>View KYC
                                                        Details</a></li>
                                            @endif
                                            @if(in_array('travel-document', $permissions))
                                                <li><a href="{{ URL::to('admin/users/travel-document/' . $allrecord->id)}}"
        ...
        <div id="info{!! $allrecord->id !!}" style="display: none;">
            <div class="nzwh-wrapper">
                <fieldset class="nzwh">
                    ...
                    <div class="drt">
                        <div class="admin_pop"><span>User Type: </span> <label>@isset($allrecord->user_type)
                        {{$allrecord->user_type}} @endisset</label></div>
                        <div class="admin_pop"><span>Full Name: </span> <label>{!! $allrecord->name !!}</label></div>
                        <div class="admin_pop"><span>Wallet Balance: </span>
                            <label>{{ CURR }}{!! number_format((($allrecord->wallet_balance - floor($allrecord->wallet_balance)) > 0.5 ? ceil($allrecord->wallet_balance) : floor($allrecord->wallet_balance)), 0, '', ' ') ?? 0  !!}</label>
                        </div>
