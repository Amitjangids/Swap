        ...
        <section id="no-more-tables" class="lstng-section">
                    ...
                    <div class="topn_left_btsec">
                        <div class="payment-info-parent">
                            <div class="payment_info">
                                ...
                                <span
                                    class="pay_body">{{ CURR }}{{  number_format((($totalAmount['total_amount'] - floor($totalAmount['total_amount'])) > 0.5 ? ceil($totalAmount['total_amount']) : floor($totalAmount['total_amount'])), 0, '', ' ') ?? 0 }}</span>
                            </div>
                            <div class="payment_info">
                                <span class="pay_head">Total Earning</span>
                                <span
                                    class="pay_body">{{ CURR }}{{  number_format((($totalAmount['total_fee'] - floor($totalAmount['total_fee'])) > 0.5 ? ceil($totalAmount['total_fee']) : floor($totalAmount['total_fee'])), 0, '', ' ') ?? 0 }}</span>
                            </div>
                        </div>
            ...
            <script>
                $('.export_excel').on('click', function () {
                    ...
                    var url = '{{ route("exportexcelexternal") }}?sender_phone=' + sender_phone + '&receiver=' + receiver + '&receiver_phone=' + receiver_phone + '&refrence=' + refrence + '&to=' + requestD;
                        ...
                        @foreach ($allrecords as $allrecord)
                            <tr>
                                <td data-title="Sender Name">
                                    {{ isset($allrecord->senderName) ? ucfirst($allrecord->senderName) : 'N/A' }}
                                </td>
                                <td data-title="Sender Phone">
                                    {{ isset($allrecord->phone) ? ucfirst($allrecord->phone) : 'N/A' }}
                                </td>
                                <td data-title="Receiver Name">
                                    {{ isset($allrecord->transaction_receiver_name) ? ucfirst($allrecord->transaction_receiver_name).' '.$allrecord->transaction_receiver_lastname : 'N/A' }}
                                </td>
                                <td data-title="Receiver Phone">
                                    {{ isset($allrecord->receiver_mobile) ? ucfirst($allrecord->receiver_mobile) : 'N/A' }}
                                </td>
                                <td data-title="Source Amount">{{ 'XAF ' . $allrecord->amount ?? 0 }}</td>
                                <td data-title="Source Amount">{{ 'XAF ' . $allrecord->transaction_amount ?? 0 }}</td>
                                <td data-title="Total Amount">

                                    {{ 'XAF ' . ($allrecord->amount + $allrecord->transaction_amount) ?? 0 }}
                ...
                <fieldset class="nzwh">
                    ...
                    <div class="drt">
                        ...
                        <div class="admin_pop"><span>Receiver Name : </span> <label>
                                {{ isset($allrecord->transaction_sender_name) ? ucfirst($allrecord->transaction_sender_name).' '.$allrecord->transaction_sender_lastname : 'N/A' }}
                            </label>
                        </div>
