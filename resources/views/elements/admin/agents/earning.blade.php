                ...
                <div class="transaction_info">
                    <div class="topn_left_btsec">
                        <div class="payment_info">
                            ...
                            <span
                                class="pay_body">{{CURR}}{{ number_format((($total['total'] - floor($total['total'])) > 0.5 ? ceil($total['total']) : floor($total['total'])), 0, '', ' ') ?? 0 }}</span>
                        </div>
                        <div class="payment_info">
                            <span class="pay_head">Total Earning</span>
                            <span
                                class="pay_body">{{CURR}}{{ number_format((($total['total_fee'] - floor($total['total_fee'])) > 0.5 ? ceil($total['total_fee']) : floor($total['total_fee'])), 0, '', ' ') ?? 0 }}</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <table class="table table-bordered table-striped table-condensed cf">
                    <thead class="cf ddpagingshorting">
                        <tr>
                            ...
                            <th class="sorting_paging">@sortablelink('payment_mode', 'Transaction Type')</th>
                    ...
                    <tbody>
                        @foreach($allrecords as $allrecord)
                                                <tr>
                                                    ...
                                                    <td data-title="Transaction ID">{{ucfirst($allrecord->User->name)}}</td>
                                                    <?php        } else { ?>
                                                    <td data-title="Transaction ID">{{ ucfirst($allrecord->Receiver->name)}}</td>
                                                    <?php        } ?>

                                                    <td data-title="Transaction ID">{{$allrecord->payment_mode}}</td>
                                                    ...
                                                    <td data-title="Transaction Fee">
                                                        {{CURR}}{{$allrecord->transaction_amount ? number_format((($allrecord->transaction_amount - floor($allrecord->transaction_amount)) > 0.5 ? ceil($allrecord->transaction_amount) : floor($allrecord->transaction_amount)), 0, '', ' ') ?? 0 : 0}}
                                                    </td>
                                                    <td data-title="Transaction Total">{{CURR}}
                                                        {{ number_format(
            ...
            <div class="nzwh-wrapper">
                <fieldset class="nzwh">
                    ...
                    <div class="drt">
                        ...
                        <div class="admin_pop"><span>Transaction Amount: </span> <label>{{CURR}}{{$allrecord->amount ? number_format((($allrecord->amount - floor($allrecord->amount)) > 0.5 ? ceil($allrecord->amount) : floor($allrecord->transaction_amount)), 0, '', ' ') ?? 0 : 0}}</label>
                        </div>
                        <div class="admin_pop"><span>Transaction Fee: </span>
                            <label>{{CURR}}{{$allrecord->transaction_amount ? number_format((($allrecord->transaction_amount - floor($allrecord->transaction_amount)) > 0.5 ? ceil($allrecord->transaction_amount) : floor($allrecord->transaction_amount)), 0, '', ' ') ?? 0 : 0}}</label></div>
                        <div class="admin_pop"><span>Transaction Total: </span>
                            <label>{{CURR}}{{ number_format(
                                                                ((($amount = (($allrecord->total_amount ?? 0) - ($allrecord->transaction_amount ?? 0)))
                                                                    - floor($amount)) > 0.5
                                                                    ? ceil($amount)
                                                                    : floor($amount)
                        ...
                        <div class="admin_pop"><span>Transaction Date: </span>
                            <label>{{$allrecord->updated_at->format('M d, Y h:i:s A')}}</label></div>
