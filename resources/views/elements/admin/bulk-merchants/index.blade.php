                    ...
                    <tbody>
                        @foreach ($allrecords as $allrecord)
                            <tr>
                                ...
                                <td data-title="Wallet Balance">{{ CURR }}{{number_format((($allrecord->wallet_balance - floor($allrecord->wallet_balance)) > 0.5 ? ceil($allrecord->wallet_balance) : floor($allrecord->wallet_balance)), 0, '', ' ') ?? 0}}
                                </td>
                                ...
                                <td data-title="KYC Status">{{ ucfirst($allrecord->kyc_status) }}</td>
                                    ...
                                    <div class="btn-group">
                                        ...
                                        <ul class="dropdown-menu pull-right">
                                            <li class="right_acdc" id="status{{ $allrecord->id }}">
                                                ...
                                                @if ($allrecord->is_verify == '1')
                                                    <a href="{{ URL::to('admin/merchants/deactivate/' . $allrecord->slug) }}"
                                                        title="Deactivate" class="deactivate"><i
                                                            class="fa fa-check"></i>Deactivate</a>
                                                @else
                                                    <a href="{{ URL::to('admin/merchants/activate/' . $allrecord->slug) }}"
                                            ...
                                            @if (in_array('edit-merchants', $permissions))
                                                <li>
                                                    <a href="{{ URL::to('admin/merchants/edit-merchants/' . $allrecord->slug) }}" title="Edit" class="">
                                            ...
                                            @if (in_array('payclient', $permissions))
                                                <li>
                                                    <a href="{{ URL::to('admin/merchants/payclient/' . $allrecord->slug) }}" title="Pay Merchant" class="">
                                                        <i class="fa fa-money"></i>Pay Merchant
                                                    </a>
                                                </li>
                                            @endif
                                            <li>
                                                <a href="#info{!! $allrecord->id !!}" title="View Merchant Detail" class="" rel='facebox'>
                                            ...
                                            @if (in_array('kycdetail', $permissions))
                                                <li>
                                                    <a href="{{ URL::to('admin/merchants/kycdetail/' . $allrecord->slug) }}" title="View KYC Details" class="">
                                                        <i class="fa fa-file"></i>View KYC Details
                                                    </a>
                                                </li>
                                            @endif
                                            --}}
                                            @if (in_array('transactionHistory', $permissions))
                                                <li><a href="{{ URL::to('admin/transactions/transactionHistory/' . $allrecord->slug) }}"
            ...
            <div class="nzwh-wrapper">
                <fieldset class="nzwh">
                    ...
                    <div class="drt">
                        ...
                        <div class="admin_pop"><span>Business Name: </span> <label>{!! $allrecord->business_name !!}</label>
                        </div>
                        <div class="admin_pop"><span>Business Owner Name: </span>
                            <label>{!! $allrecord->name !!}</label>
                        </div>
                        <div class="admin_pop"><span>Wallet Balance: </span> <label>{!! CURR . ' ' . number_format((($allrecord->wallet_balance - floor($allrecord->wallet_balance)) > 0.5 ? ceil($allrecord->wallet_balance) : floor($allrecord->wallet_balance)), 0, '', ' ') ?? 0 !!}</label>
                        </div>
                        ...
                        <div class="admin_pop"><span>Date Of Birth: </span> <label>{!! $allrecord->dob !!}</label>
                        </div>
                        @if ($allrecord->profile_image != '')
                            <div class="admin_pop"><span>Profile Image</span>
                                <label>{{ HTML::image(PROFILE_FULL_DISPLAY_PATH . $allrecord->profile_image, SITE_TITLE, ['style' => 'max-width: 200px']) }}</label>
                            </div>
                        @endif
                </fieldset>
            </div>
