...
<div class="col-sm-4">
    <div class="agent-box">
        <div class="agent-detail-small">
            ...
            <div class="agent-pic">
                @if(isset($allrecord->Receiver->profile_image) && !empty($allrecord->Receiver->profile_image))
                            {{HTML::image(PROFILE_SMALL_DISPLAY_PATH.$allrecord->Receiver->profile_image, SITE_TITLE, ['id'=> ''])}}
                            @else
                            {{HTML::image('public/img/no_user.png', SITE_TITLE, ['id'=> ''])}}
                            @endif
            </div>
            <div class="agent-namebox">
                <h5>{{$allrecord->Receiver->name}}</h5>
                <h6>{{$allrecord->Receiver->phone}}</h6>
            </div>
            <div class="requst_btn">
                <a href="javascript:void(0);" onclick="selectRequest('{{$allrecord->id}}', 'Reject','{{$allrecord->amount}}')"><span>{{__('message.Reject')}}</span></a>
                <a href="javascript:void(0);" onclick="selectRequest('{{$allrecord->id}}', 'Accept','{{$allrecord->amount}}')"><span>{{__('message.Accept')}}</span></a>
            </div>
        </div>
    </div>
</div>
